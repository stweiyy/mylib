...
main<-function(){
	...
	querysql<-sprintf("SELECT DISTINCT URL  FROM %s.TB_COM_DIM_XDR  where URL not in (SELECT DISTINCT URL from %s.TB_POI_CAL_URL_WORD) limit 1000",config$schema,config$schema)
