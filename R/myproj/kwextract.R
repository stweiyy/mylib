...
#main function
main<-function(logfile="kwextract.log"){
	...
	v<-lapply(urlvec,function(url){
			  ...
			  print(content)
			  keywords<-textRankExtract(content,stopwords)
			  print(keywords)
			  return(NULL)
	          tryCatch({
			    		upsertstat<-sprintf("UPSERT %s.TB_POI_CAL_URL_CONTENT VALUES('%s','%s','%s','%s') where URL ='%s'",config$schema,url,content,"zh",Sys.Date(),url)
